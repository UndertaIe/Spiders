...
def getProxy():
    ...
    try:
        aproxy = requests.get(url,headers={"UserAgent":"-*-SearchSpider-*-"}).json()
...
def getAllProxy():
    ...
    try:
        proxies = requests.get(url,headers={"UserAgent":"-*-SearchSpider-*-"}).json()
    except ConnectionError:
...
def getProxyCount():
    ...
    try:
        count = requests.get(url,headers={"UserAgent":"-*-SearchSpider-*-"}).json()
    except ConnectionError:
