...
class RedisSpiderSmartIdleClosedExtensions(object):
    ...
    def spider_opened(self, spider):
        if spider.search == "":
            logger.info("###[INFO] Cli Spider without search arg,Spider will get select_urls from Redis ###")
            rhandler = RedisHandler()
            key = rhandler.get_redis_select_key()
            if key is not None:
    ...
    def spider_idle(self, spider):
        ...
        # 连续空闲次数超过设置的reset_key_number并且singleSearch为False时则重置redis_key
            ...
            if key is not None:
                ...
                logger.info("###[INFO] Spider current redis_key: <{}>. Search: <{}> ###".format(spider.redis_key,spider.search))
            rhandler.close()
        # 连续空闲次数大于idle_number时关闭爬虫程序
        else:
            # 执行关闭爬虫操作
            self.crawler.engine.close_spider(spider, 'BossmSlaveSpider')